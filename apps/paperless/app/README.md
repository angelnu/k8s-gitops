...
| `superuser_sync.py` | `m2m_changed` — mirrors the admin SSO group onto `is_superuser` at login |
...
## Superuser reconcile: two paths

`sync_superuser` handles the common case — group changes in authentik take
effect on the user's next login, because group sync runs at login and fires
`m2m_changed`.

`provision_extra_superadmins` handles the case that signal misses: a full
pass at container start. When `SSO_GROUP_ADMIN` itself changes, or the
extra-superadmin list changes, existing users' *membership* does not change,
so no signal fires and re-login has no effect. **Changing `SSO_GROUP_*`
requires a pod restart, not just a re-login.**
